...
alloc_proc(void) {
     ...
     */
    //LAB8:EXERCISE2 2013011325 HINT:need add some code to init fs in proc_struct, ...
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011325
    //LAB8:EXERCISE2 2013011325  HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2013011325  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
	...
	struct proghdr ph;
	int phi = 0;
	for (phi = 0; phi < elf.e_phnum; ++phi) {
		off_t phoff = elf.e_phoff + phi * sizeof(struct proghdr);
		...
		*/
		...
		while (start < end) {
			...
			int off = start - la, size = PGSIZE - off;
			la += PGSIZE;
			if (end < la) {
				size -= (la - end);
			}
	...
	*/
	...
	lcr3(PADDR(mm->pgdir));
	//    * (6) setup uargc and uargv in user stacks
	int i; int argv_size = 0; char ** uargv;
	for (i = 0; i < argc; ++i) {
		argv_size += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
	}
	uint32_t stacktop = USTACKTOP - (argv_size/sizeof(long)+1)*sizeof(long);
	uargv = (char**) (stacktop - argc*sizeof(char*));
